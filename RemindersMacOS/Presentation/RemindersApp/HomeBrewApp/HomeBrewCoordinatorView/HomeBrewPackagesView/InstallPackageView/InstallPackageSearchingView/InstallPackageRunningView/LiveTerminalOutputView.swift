...
struct LiveTerminalOutputView: View {
    ...
    var body: some View {
        if showRealTimeTerminalOutputOfOperations {
            ...
            .onAppear { /// This has to be here so that the real-time output dropdown can be open according to user preference, while, at the same time, enabling the user to open/close the dropdown without also changing their preference in the process
                isRealTimeTerminalOutputExpanded = openRealTimeTerminalOutputByDefault
            }
            .onDisappear {
                if !forceKeepTerminalOutputInMemory {
                    AppLogger.homeBrew.log("Purging saved real time output")
                    lineArray = .init()
                } else {
                    AppLogger.homeBrew.log("Forced to keep previous output in memory")
                }
            }
        }
    }
...
/*#Preview {
    LiveTerminalOutputView(lineArray: .init(), isRealTimeTerminalOutputExpanded: true)
}*/
