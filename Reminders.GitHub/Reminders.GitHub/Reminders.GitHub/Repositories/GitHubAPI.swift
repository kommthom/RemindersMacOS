...
public let GitHubAPI = _GitHubAPI()

public class _GitHubAPI: GitHubAPIProtocol {
    ...
    public func activeRepositories(filterCriteria: GitHubFilterCriteria) -> Resource {
        return service
            .resource("/search/repositories")
            .withParams([
                "q": "\(filterCriteria.titleContains) language:\(filterCriteria.language) stars:>0",
                "sort": "updated",
                "order": "desc"
            ])
    }

    public func user(_ username: UserName) -> Resource {
        return service
            .resource("/users")
            .child(username.userName.lowercased())
    }
    
    public func userRepositories(resourceUrlString: ResourceUrlString) -> Resource {
        return resourceUrlString.resource
            .optionalRelative(resourceUrlString.urlString)?
            .withParam("sort", "updated") ?? resourceUrlString.resource
    }

    public func repository(_ loginRepositoryName: LoginRepositoryName) -> Resource {
        return service
            .resource("/repos")
            .child(loginRepositoryName.login)
            .child(loginRepositoryName.repositoryName)
    }

    public func repository(_ repositoryModel: Repository) -> Resource {
        return repository(.init(ownedBy: repositoryModel.owner.login, named: repositoryModel.name))
    }

    public func currentUserStarred(_ loginRepositoryName: LoginRepositoryName) -> Resource {
        return service
    ...
    public func setStarred(_ loginRepositoryNameIsStarred: LoginRepositoryNameIsStarred) -> Request {
        let starredResource = currentUserStarred(.init(ownedBy: loginRepositoryNameIsStarred.login, named: loginRepositoryNameIsStarred.repositoryName))
        return starredResource
            .request(loginRepositoryNameIsStarred.isStarred ? .put : .delete)
            .onSuccess { _ in
                // Update succeeded. Directly update the locally cached “starred / not starred” state.

                starredResource.overrideLocalContent(with: loginRepositoryNameIsStarred.isStarred)
    ...
    public func optionalRelative(resourceUrlString: ResourceUrlString) -> Resource {
        return resourceUrlString.resource.optionalRelative(resourceUrlString.urlString) ?? resourceUrlString.resource
    }
}
