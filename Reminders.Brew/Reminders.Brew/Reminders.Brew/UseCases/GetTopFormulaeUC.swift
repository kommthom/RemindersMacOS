...
public class DefaultGetTopFormulaeUC: GetTopFormulaeUCProtocol {
    ...
    public func execute(numberOfDays: Int, topPackages: LoadableSubject<[TopPackage]>) -> Void {
        ...
        Just<Void>
            ...
            .flatMap { [homeBrewWebRepository] in
                homeBrewWebRepository.topPackages(isCask: false, numberOfDays: numberOfDays)
            }
...
public class MockedGetTopFormulaeUC: GetTopFormulaeUCProtocol {
    public func execute(numberOfDays: Int, topPackages: LoadableSubject<[TopPackage]>) -> Void {
        let cancelBag = CancelBag()
        topPackages.wrappedValue.setIsLoading(cancelBag: cancelBag)
    }
